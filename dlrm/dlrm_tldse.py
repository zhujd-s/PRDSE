...
class DLRM_Net(nn.Module):
    def create_mlp(self, ln, sigmoid_layer, softmax = False, bndrop = False):
        ...
        for i in range(0, ln.size - 1):
            ...
            if i == sigmoid_layer:
                ...
                if(bndrop): layers.append(nn.BatchNorm1d(m))
                layers.append(nn.ReLU())
                if(bndrop): layers.append(nn.Dropout(0.2))
            else:
            	pass # without activation function
        ...
        if (
            ...
            if ndevices <= 1:
                ...
                if self.weighted_pooling == "learned":
                    ...
                    self.v_W_l = w_list
            self.bot_l = self.create_mlp(ln_bot, sigmoid_bot, bndrop = True)
            self.top_l = self.create_mlp(ln_top, sigmoid_top, softmax = True, bndrop = True)
...
class dlrm_module():
    def __init__(self, m_out, den_idx, spa_idx, spa_shape, iindex = 0, rtype = "none"):
        self.den_idx = den_idx
        self.spa_idx = spa_idx
        m_den = len(self.den_idx)
        #m_spa = len(self.spa_idx)
        m_spa = 16
        #module
        #m_spa = args.arch_sparse_feature_size=16
        #ln_emb = args.arch_embedding_size=4-3-2
        #ln_bot = args.arch_mlp_bot="13-512-256-64-16"
        #ln_top = arch_mlp_top_adjusted="512-256-1"
        #arch_embedding_size="4-3-2"
        arch_mlp_bot="128-64" #require to fill in input and out deminsion
        arch_mlp_top="256-128-128-128-64" #require to fill in output deminsion
        arch_interaction_op = "dot"#"dot"#"cat"
        arch_interaction_itself = False

        #ln_emb = np.asarray(np.fromstring(arch_embedding_size, dtype=int, sep="-"))
        ln_emb = np.asarray(spa_shape)
        
        arch_mlp_bot_adjusted = str(m_den) + "-" + arch_mlp_bot + "-" + str(m_spa)
        ln_bot = np.fromstring(arch_mlp_bot_adjusted, dtype=int, sep="-")
        ...
        arch_mlp_top_adjusted = str(num_int) + "-" + arch_mlp_top + "-" + str(m_out)
        ln_top = np.fromstring(arch_mlp_top_adjusted, dtype=int, sep="-")
        ...
        self.optimizer = opts["sgd"](self.dlrm.parameters(), lr=0.01)
    ...
    def forward(self, X, lS_o, lS_i):
        return self.dlrm(X, lS_o, lS_i)

    def train(self, obs_list, reward_list):
        #if(self.rtype == "discreet_phd"):
        #    print(f"*********   {self.iindex}_{self.rtype}_start train fillter   **********************")
        #data profiling
        #random.shuffle(obs_list)
        #buffer reorder
        data_size = len(obs_list)
        self.training_epochs = data_size
        sample_list = list(zip(obs_list, reward_list))
        random.shuffle(sample_list)
        sample_list = list(zip(*sample_list))
        obs_list = list(sample_list[0])
        reward_list = list(sample_list[1])
        ...
        # training iteration
        #dlrm原生代码中的输入格式比较特别
        #X表示dense input，其矩阵格式为(行索引=batch_index, 列索引=dense_input)
        #lS_i表示sparse input, 其类型为list，list中的某个item包含某个sparse variable的所有batch，其矩阵格式为(行索引=spare_input, 列索引=batch_index)
        #lS_o表示sparse input的offset，其矩阵格式为(行索引=sparse_input, 列索引=batch_index)
        #所以lS_i需要额外的转置
        for epoch in range(self.training_epochs):
            idxs = np.random.randint(0, int(0.8*data_size), size=self.batch_size)
            data = data_all[idxs]
            X = data[:, self.den_idx]
            lS_i = data[:, self.spa_idx].T
            offset = range(self.batch_size) #in TLDSE, offset is always 1
            offset_list = [offset for row in range(len(self.spa_idx))]
            offset_all = np.array(offset_list)  
            lS_o = offset_all
            T = target_all[idxs]

            X = torch.as_tensor(X, dtype=torch.float32)
            lS_o = torch.as_tensor(lS_o, dtype=torch.int64)
            lS_i = torch.as_tensor(lS_i, dtype=torch.int64)
            #T = torch.as_tensor(T, dtype=torch.float32)
            T = torch.as_tensor(T, dtype=torch.int64)

            self.dlrm.train()
            Z = self.forward(X, lS_o, lS_i)
            #Z = torch.nn.functional.softmax(Z, dim = -1)
            E = self.dlrm.loss_fn(Z, T)
        ...
        data = data_all[test_range]
        X = data[:, self.den_idx]
        lS_i = data[:, self.spa_idx].T
        offset = range(len(test_range))
        offset_list = [offset for row in range(len(self.spa_idx))]
        offset_all = np.array(offset_list)
        lS_o = offset_all
        ...
        self.dlrm.eval()
        with torch.no_grad():
            predict = self.forward(X, lS_o, lS_i)
        t_predict = list()
        for i in predict:
            if(i[0] > 0.9): t_predict.append(0)
            #if(i[0] > i[1]): t_predict.append(0)
            else: t_predict.append(1) 
        t_target = target_all[test_range]
        error = 0
        for i,j in zip(t_predict, t_target):
            #if(i != j[0]): 
            if(i != j):
                error += 1
        if(self.rtype == "discreet_phd"): print(f"************ iindex:{self.iindex},  accucy = {1 - error/len(test_range)}    ***************")
    
    def predict(self, obs):
        data = np.array(obs)
        X = data[self.den_idx]
        lS_i = data[self.spa_idx].T
        offset = range(1) #in TLDSE, offset is always 1
        offset_list = [offset for row in range(len(self.spa_idx))]
        offset_all = np.array(offset_list)  
        lS_o = offset_all
        ...
        lS_o = torch.as_tensor(lS_o, dtype=torch.int64)
        lS_i = torch.as_tensor(lS_i, dtype=torch.int64).reshape(len(self.spa_idx), 1)
...
class dlrm_encoder():
    def __init__(self, m_out, den_idx, spa_idx, spa_shape):
        self.den_idx = den_idx
        self.spa_idx = spa_idx
        self.dim = m_out
        m_den = len(self.den_idx)
    ...
    def train(self, sample_list):
        print(f"**************   start train encoder   **********************")
        data_size = len(sample_list)
        random.shuffle(sample_list)
        dim = self.dim
        ...
        def get_ub(sample_list, key):
            ub = 0
            for sample in sample_list:
                value = sample[key]
                if(value > ub): ub = value
            return ub
        ub = get_ub(sample_list, "power")
        stn_power = ub / dim
        ...
        for sample in sample_list:
            power = sample["power"]
            if(power < ub):
                power_label =  int(power // stn_power)
                sample["power_lable"] = power_label
            else:
                sample["power_lable"] = dim - 1

            data_list.append(sample["obs"])
            power_target_list.append(power_label)

        data_all = np.array(data_list)
        target_all = np.array(power_target_list)
        ...
        #所以lS_i需要额外的转置
        for epoch in range(self.training_epochs):
            idxs = np.random.randint(0, int(0.9*data_size), size=self.batch_size)
            data = data_all[idxs]
            X = data[:, self.den_idx]
            lS_i = data[:, self.spa_idx].T
            offset = range(self.batch_size) #in TLDSE, offset is always 1
            ...
            if epoch % 100 == 0 and epoch != 0:
                # acurrancy test
                #print(f"current_epoch:{epoch}, loss:{L}")
                pass
            if epoch % 100 == 0 and epoch != 0:    
                test_range = list(range(int(0.9 * data_size), data_size))
                data = data_all[test_range]
                X = data[:, self.den_idx]
                lS_i = data[:, self.spa_idx].T
                offset = range(len(test_range))
                ...
                t_predict = list()
                for i in predict:
                    t_predict.append(int(torch.argmax(i).item()))
                t_target = target_all[test_range]
                error = 0
